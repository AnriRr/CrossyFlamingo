...
const Config = {
  ...
  // Side-angle perspective: horizontal shear (no vertical squish)
  // Positive value = rows shift right as Y increases (front-right view angle)
  ISO_SHEAR:    0.45,
...
class Player {
  ...
  reset() {
    const W = window.innerWidth;
    const H = window.innerHeight;
    const startY = H * Config.TARGET_Y;
    this.x      = W / 2;
    this.y      = startY;
    this.tx     = W / 2;
    this.ty     = startY;
...
class Game {
  ...
  _update(dt) {
    ...
    // World scrolls AFTER forward hop lands — not at hop-start
  ...
  _draw() {
    ...
    // Side-angle perspective: horizontal shear only — no squish.
    // ctx.transform(1, 0, shearX, 1, 0, 0) shifts each pixel's X by shearX * Y.
    // Rows at the top of screen (small Y) shift left; bottom rows shift right.
    // This gives the appearance of viewing the grid from a front-right angle.
    ctx.save();
    ctx.transform(1, 0, Config.ISO_SHEAR, 1, 0, 0);
  ...
  _checkCollisions() {
    ...
    // Fell off bottom
    if (py > window.innerHeight + this.tileSize) this._die('fell into the abyss');
  }
