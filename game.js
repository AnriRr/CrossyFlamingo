...
const CFG = {
  ...
  LANE_DEPTH:  22,    // px height of the dark side face below each lane
  COLS:        9,     // number of obstacle columns (determines obstacle x spacing)

  // Camera: player sits at PLAYER_SCREEN_ROW lanes from bottom
  PLAYER_SCREEN_ROW: 3,

  // Hop animation
  HOP_SPEED:   10,    // lanes per second
  HOP_HEIGHT:  52,    // max pixel lift during hop arc

  // World
  ROWS_ABOVE:  14,    // lane rows to keep above player
  ROWS_BELOW:  5,     // lane rows to keep below player
...
class World {
  constructor() {
    this.lanes      = [];   // array of lane objects
    this.playerRow  = 0;    // which world row the player is on
    this.highestRow = 0;
  }

  get laneStep() { return CFG.LANE_H + CFG.LANE_DEPTH; }

  // Screen Y of the TOP of the lane's face for a given worldRow
  laneScreenY(worldRow, canvasH) {
    const playerScreenY = canvasH - CFG.PLAYER_SCREEN_ROW * this.laneStep;
    return playerScreenY - (worldRow - this.playerRow) * this.laneStep;
  }

  makeLane(worldRow, score) {
    const r   = Math.random();
    ...
    // Spawn obstacles — x positions in pixels across screen width
    // We'll set them at render time based on canvas width, store as fraction 0..1
  ...
  reset(score) {
    ...
    for (let i=0; i<CFG.ROWS_ABOVE+CFG.ROWS_BELOW; i++) {
      this.lanes.push(this.makeLane(i, score));
    }
  }
  ...
  update(dt, score, frame, W) {
    ...
    while (this.lanes.length < CFG.ROWS_ABOVE + CFG.ROWS_BELOW) {
      this.highestRow++;
      this.lanes.push(this.makeLane(this.highestRow, score));
    }
  }
...
class Player {
  ...
  move(dir, W) {
    ...
    this.hopDist  = Math.sqrt((this.tpx-this.px)**2 + (this.trow-this.row)**2) || 1;
  ...
  update(dt) {
    ...
    if (this.moving) {
      const spd   = CFG.HOP_SPEED * dt;
      const dpx   = this.tpx  - this.px;
      const drow  = this.trow - this.row;
      const dist  = Math.sqrt(dpx*dpx + drow*drow);

      if (dist < spd*1.2 || dist < 0.02) {
        this.px   = this.tpx;
        this.row  = this.trow;
        this.moving = false;
        this.hop    = 0;
        ...
        this.px  += dpx  * t;
        this.row += drow * t;
        this.hop  = Math.sin(Math.PI * (1 - dist/this.hopDist)) * 0.9;
      }
    }
    return done;
  }
...
class Renderer {
  ...
  drawLane(lane, topY) {
    ...
    // ── Bottom side face (dark, gives 3D depth) ──
    ctx.fillStyle = darken(lane.topColor, 0.45);
  ...
  _constructionStripes(topY, H) {
    ...
    for (let x=-20; x<this.W+20; x+=22) {
      ctx.beginPath();
      ctx.moveTo(x, topY);
      ctx.lineTo(x+14, topY);
      ctx.lineTo(x+6, topY+H);
      ctx.lineTo(x-8, topY+H);
      ctx.closePath();
      ctx.fill();
    }
  }
  ...
  drawPlayer(player, charIdx, world) {
    ...
    const topY  = world.laneScreenY(Math.round(player.row), this.H);
    const hopPx = player.hop * CFG.HOP_HEIGHT;

    // player center x = player.px, center y = lane top + half lane height
    const cx = player.px;
