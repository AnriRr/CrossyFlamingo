...
const CFG = {
  // Lane geometry
  LANE_H:      72,    // px height of each lane strip (top face)
  LANE_DEPTH:  28,    // px height of the dark side face below each lane
  COLS:        9,     // number of obstacle columns (determines obstacle x spacing)

  // Camera
  PLAYER_SCREEN_ROW: 3,    // player sits this many lanes from bottom
  CAMERA_SPEED:      6.0,  // how fast camera catches up (rows/sec lerp rate)

  // Side-angle shear: each lane's top-left corner shifts right by SHEAR * (laneStep * rowsFromBottom)
  // This makes further lanes appear offset to the right — like viewing from front-left
  ISO_SHEAR:   0.55,   // horizontal pixels shift per vertical pixel of lane offset

  // Hop animation — everything in PIXELS now, not mixed units
  HOP_PX_SPEED: 900,  // pixels per second during hop (same for all directions)
  HOP_HEIGHT:   52,   // max pixel lift during hop arc
...
class World {
  ...
  // Screen Y of the TOP of a lane's face, using smooth cameraRowF
  // Also returns the horizontal shear offset for this lane
  ...
  // Shear offset: lanes further up (higher worldRow = further away) shift LEFT
  // Lanes closer (lower worldRow) shift RIGHT → gives front-left viewing angle
  laneShearX(worldRow, canvasH) {
    const playerScreenY = canvasH - CFG.PLAYER_SCREEN_ROW * this.laneStep;
    const topY = this.laneScreenY(worldRow, canvasH);
    // How many px above the player's row center is this lane?
    const offsetFromPlayer = (playerScreenY + CFG.LANE_H/2) - (topY + CFG.LANE_H/2);
    return offsetFromPlayer * CFG.ISO_SHEAR;
  }
  ...
  update(dt, score, frame, W) {
    ...
    this.lanes = this.lanes.filter(l => l.worldRow >= this.playerRow - CFG.ROWS_BELOW);
...
class Renderer {
  ...
  drawLane(lane, topY, shearX) {
    ...
    // Shear: top edge shifts by +shearX, bottom edge by 0
    // This draws each lane as a parallelogram (side-angle look)
    const shTop = shearX;   // top-left x offset
    const shBot = 0;        // bottom-left x offset (anchor)

    // ── Top face (parallelogram) ──
    ctx.fillStyle = lane.topColor;
    ctx.beginPath();
    ctx.moveTo(shBot,       topY + H);   // bottom-left
    ctx.lineTo(W + shBot,   topY + H);   // bottom-right
    ctx.lineTo(W + shTop,   topY);       // top-right
    ctx.lineTo(shTop,       topY);       // top-left
    ctx.closePath();
    ctx.fill();

    // ── Bottom side face (parallelogram, darker) ──
    ctx.fillStyle = darken(lane.topColor, 0.42);
    ctx.beginPath();
    ctx.moveTo(shBot,       topY + H);        // top-left of face
    ctx.lineTo(W + shBot,   topY + H);        // top-right
    ctx.lineTo(W + shBot,   topY + H + D);    // bottom-right
    ctx.lineTo(shBot,       topY + H + D);    // bottom-left
    ctx.closePath();
    ctx.fill();

    // ── Top edge highlight ──
    ctx.fillStyle = lighten(lane.topColor, 0.18);
    ctx.beginPath();
    ctx.moveTo(shTop,     topY);
    ctx.lineTo(W+shTop,   topY);
    ctx.lineTo(W+shBot,   topY+2);
    ctx.lineTo(shBot,     topY+2);
    ctx.closePath();
    ctx.fill();

    // ── Lane markings ──
    ctx.save();
    // Clip to the lane parallelogram so markings don't bleed
    ctx.beginPath();
    ctx.moveTo(shBot, topY+H); ctx.lineTo(W+shBot, topY+H);
    ctx.lineTo(W+shTop, topY); ctx.lineTo(shTop, topY);
    ctx.closePath(); ctx.clip();

    switch(lane.type) {
      case 'road':         this._roadLines(lane, topY, H, shearX);  break;
      case 'water':        this._waterShimmer(lane, topY, H);       break;
      case 'grass':        this._grassTufts(lane, topY, H, shearX); break;
      case 'construction': this._constructionStripes(topY, H);      break;
    }

    // ── Obstacles ──
    lane.obs.forEach(ob => this._drawOb(ob, lane, topY, H, shearX));

    // ── Collectible ──
    if (lane.collectible && !lane.collectible.collected) {
      const rawX = lane.collectible.frac * W;
      // shear interpolation: at topY+H/2 (lane center vertically)
      const midFrac = 0.5;  // lane vertical center
      const cx = rawX + shearX * (1 - midFrac);
      const icons = ['💰','📄','📰','🦐','🪶','💰'];
      const bob   = Math.sin(this.f*0.09 + rawX)*4;
      ctx.font=`${H*0.58}px serif`;
      ctx.textAlign='center'; ctx.textBaseline='middle';
      ctx.fillText(icons[Math.floor(rawX/80)%icons.length], cx, topY+H/2+bob-4);
    }

    ctx.restore();
  }

  _roadLines(lane, topY, H, shearX) {
    const ctx = this.ctx;
    ctx.fillStyle='rgba(255,255,255,0.16)';
    const dw=28, gap=56;
    const scroll = (this.f * lane.speed * lane.dir * 0.008) % gap;
    for (let x=-gap; x<this.W+gap; x+=gap) {
      // shear 0.5 = lane vertical midpoint
      ctx.fillRect(x + scroll + shearX*0.5, topY+H/2-2, dw, 4);
    }
  }
  ...
  _grassTufts(lane, topY, H, shearX) {
    const ctx = this.ctx;
    ctx.fillStyle='rgba(0,0,0,0.10)';
    for (let x=10; x<this.W; x+=28+(lane.worldRow*7%14)) {
      const sx = x + shearX*0.5;
      ctx.fillRect(sx,   topY+H*0.55, 3, 10);
      ctx.fillRect(sx+7, topY+H*0.48, 2, 13);
    }
  }
  ...
  /* ── Obstacle block ─────────────────────────────────── */
  _drawOb(ob, lane, topY, H, shearX) {
    const ctx  = this.ctx;
    const rawX = ob.frac * this.W;
    const cx   = rawX + shearX * 0.5;  // shear at lane vertical midpoint
  ...
  drawPlayer(player, charIdx, world) {
    ...
    const shearX = world.laneShearX(player.row, this.H);
    const hopPx  = player.hop * CFG.HOP_HEIGHT;

    // player center: px shifted by shear at lane midpoint
    const cx = player.px + shearX * 0.5;
...
class Game {
  ...
  _draw() {
    ...
    // Draw lanes back-to-front (highest worldRow = furthest away = draw first)
    const sorted = [...this.wld.lanes].sort((a,b) => b.worldRow - a.worldRow);
    sorted.forEach(lane => {
      const topY   = this.wld.laneScreenY(lane.worldRow, this.rnd.H);
      const shearX = this.wld.laneShearX(lane.worldRow, this.rnd.H);
      this.rnd.drawLane(lane, topY, shearX);
    });
