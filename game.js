...
const Config = {
  ...
  // Isometric camera
  ISO_Y:        0.58,   // vertical squish factor (1 = flat top-down, 0.5 = strong iso)
  ISO_ORIGIN_Y: 0.82,   // screen-fraction where the iso ground plane anchors
...
class Player {
  ...
  move(direction, tileSize) {
    ...
    // Forward (up): world scrolls — player stays at same screen Y.
    // Backward (down): world doesn't scroll — player moves down one row visually.
    const dy = direction === 'down' ? tileSize : 0;
...
class Renderer {
  ...
  drawRow(row, y, tileSize, frameCount) {
    ...
    const sideH = Math.round(tileSize * 0.32);
...
class Game {
  ...
  /* ── Hop advancement ────────────────────────────────── */
  advanceWorld(dir) {
    this.world.worldOffset += this.tileSize * dir;
    if (dir > 0) {
      this.score++;
      this.ui.updateScore(this.score);
      this.ui.updateEra(this.score);
    }
    // Going backward: worldOffset shrinks, no score change
  }
...
Player.prototype.move = function(direction, tileSize) {
  ...
  if (direction === 'down') Game.instance.advanceWorld(-1);
};
