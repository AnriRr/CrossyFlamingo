...
class World {
  ...
  /** Advance the world by one tile (player hopped forward) */
  advance() {
    this.worldOffset += this.tileSize;
  }
...
class Player {
  ...
  reset() {
    ...
    const H = window.innerHeight;
    this.x      = W / 2;
    this.y      = H * Config.TARGET_Y;
    this.tx     = W / 2;
    this.ty     = H * Config.TARGET_Y;
    this.moving = false;
    this.alive  = true;
    this.hop    = 0;       // 0–1 arc height factor
    this.hopDir = 0;       // -1 left, 0 forward, 1 right
    this.squish = 0;       // landing squash
    this.trail  = [];
  }
  ...
  update(dt, tileSize) {
    ...
    if (this.moving) {
      ...
      if (dist < speed * 1.5 || dist < 2) {
        ...
        this.hop = Math.sin(Math.PI * (1 - dist / tileSize)) * 0.45;
      }
    }

    // Drift on logs / yachts
    // (applied externally by Game after world update)
...
class Renderer {
  ...
  drawRow(row, y, tileSize, frameCount) {
    const ctx = this.ctx;
    if (y < -tileSize || y > this.H + tileSize) return;
...
class Game {
  ...
  /* ── Hop advancement ────────────────────────────────── */
  // Called from Player.move when direction is 'up'
  advanceWorld() {
    this.world.advance();
    this.score++;
  ...
  _checkCollisions() {
    ...
    // Fell off bottom
    if (py > window.innerHeight + tile) this._die('fell into the abyss');
  }
...
/* ── Patch Player.move to trigger world advance ─────── */
...
Player.prototype.move = function(direction, tileSize) {
  ...
  if (direction === 'up' && this.moving) {
    Game.instance.advanceWorld();
  }
};
